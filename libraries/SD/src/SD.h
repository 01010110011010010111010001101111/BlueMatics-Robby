...
class SDFile : public Stream { // DAE
 private:
  char _name[13]; // our name
  SdFile *_file;  // underlying file pointer

public:
  SDFile(SdFile f, const char *name);     // wraps an underlying SdFile // DAE
  SDFile(void);      // 'empty' constructor // DAE
  ...
  SDFile openNextFile(uint8_t mode = O_RDONLY); // DAE
...
public:
  ...
  SDFile open(const char *filename, uint8_t mode = FILE_READ); // DAE
  SDFile open(const String &filename, uint8_t mode = FILE_READ) { return open( filename.c_str(), mode ); } // DAE
