...
SDFile SDClass::open(const char *filepath, uint8_t mode) { // DAE
  /*
  ...
  return SDFile(file, filepath); // DAE
}
...
SDFile SDFile::openNextFile(uint8_t mode) { // DAE
  dir_t p;
...
void SDFile::rewindDirectory(void) {   // DAE
  if (isDirectory())
    _file->rewind();
}
